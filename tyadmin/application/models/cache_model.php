	...
	function handleModelData($data){
		...
		foreach($data as $item){
			if(isset($item['id'])){
                ...
                $diffTime = time()-$item['createtime'];

                if($diffTime <= 3600) {
                    //处理分钟
                    $item['diffTime'] = ceil($diffTime/60).'分钟前';
                }
                else if($diffTime <= 3600 * 24 && $diffTime > 3600) {
                    //处理小时
                    $item['diffTime'] = ceil($diffTime/3600).'小时前';
                }
                else {
                    //处理天
                    $item['diffTime'] = ceil($diffTime/(3600*24)).'天前';
                }
				$list[] = $item;
			}else{
				$list[] = array();
